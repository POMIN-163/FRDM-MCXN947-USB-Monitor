...
static rt_sem_t s_memWriteDone = RT_NULL;
#else
...
#if defined(SDK_OS_FREE_RTOS)
        rt_err_t code = rt_sem_take(s_memWriteDone, 10);
        if (code != RT_EOK)
        {
...
status_t DEMO_InitLcdController(void)
{
    ...
    const st7796s_config_t st7796sConfig = {.driverPreset    = kST7796S_DriverPresetLCDPARS035,
                                            ...
                                            .orientationMode = kST7796S_Orientation90,
...
#if defined(SDK_OS_FREE_RTOS)
    s_memWriteDone = rt_sem_create("lv_sem", 0, RT_IPC_FLAG_PRIO);
